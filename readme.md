...
任何代码修改、构建、启动、停止、重启、部署或排障操作前，必须先完整阅读本文件，并检查当前 Git 工作区状态。不得跳过本节或仅凭历史记忆执行操作。

- 运行产物以 `build/<服务>/` 为准；源码修改后必须重新构建，再重启对应服务。
- 外网 Web 通过 HTTPS 反向代理和随机路径访问；修改静态资源或路径逻辑后，必须同时检查 HTTPS、Nginx 反代和浏览器缓存影响。
- 变更前保留用户已有修改，不得使用破坏性 Git 命令覆盖工作区。
- 操作完成后必须验证服务状态、关键接口和 Git diff；如涉及登录，必须实际验证登录请求及跳转链路。
