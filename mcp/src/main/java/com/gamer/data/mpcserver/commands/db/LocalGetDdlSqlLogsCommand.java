...
/**
 * 获取 DDL SQL 日志（依赖 MySQL general_log）。
 */
@Process("local_get_ddl_sql_logs")
public class LocalGetDdlSqlLogsCommand implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        String sql = "SELECT event_time, user_host, thread_id, argument "
            ...
            + "argument LIKE 'TRUNCATE %' OR "
            + "argument LIKE 'RENAME %'"
            + ") "
        ...
        try {
            ...
            return CommandResult.of("DDL 日志获取失败/不可用: " + e.getMessage());
        } finally {
